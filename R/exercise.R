...
GetFromTreeBase_exercise <- function() {
	...
	print("This is the biggest tree")
...
# "Error in scan(file=file, what = "". sep = "\n, quiet=TRUE), this error occurs with the search_treebase() function
...
GetTreeFromOpenTree_exercise <- function() {
	...
	#anolis.tree <- tol_subtree(ott_id=5342306)
	...
	anolis.studies.id<- anolis.studies[[1]][[1]]
	
	# Let's get info on the first study
	anolis.study1.metadata <- get_study_meta(anolis.studies.id)
	...
	#anolis.study1.tree1<- studies_find_trees(property="ot:studyId", 	value="pg_2881", verbose=TRUE, detailed=FALSE)
	
	#anolis.tree.id<-anolis.study1.tree1[[3]]
	
	#studies_properties() #list of properties that can be used in the 	search
	# And plot it
	plot.phylo(anolis.study1.tree1, type="fan", cex=0.2)
	
	#Return both trees
	return.list <- list(anolis.tree, anolis.study1.tree1)
	names(return.list) <- c("anolis.tree", "anolis.study1.tree1")
	return(return.list)
}